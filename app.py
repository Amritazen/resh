...
def upload_file():
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                ...
                if "quota" in err or "429" in err or "resource_exhausted" in err:
                    return jsonify({"success": False, "error": "The AI service is busy right now. Please try uploading again in a moment."})
                if "api_key" in err or "invalid" in err:
                    return jsonify({"success": False, "error": "Configuration error. Please contact the site owner."})
                return jsonify({"success": False, "error": "Could not process this file. Please try again or use a different file."})
        ...
        err = str(e).lower()
        if "quota" in err or "429" in err or "resource_exhausted" in err:
            return jsonify({"success": False, "error": "The AI service is busy right now. Please try again in a moment."})
